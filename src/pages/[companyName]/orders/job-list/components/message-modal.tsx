...
const Message = ({
  ...
  const jobTerminatedLPMStatuses = [
    60600, 60700, 60800, 60900, 601000, 601100, 70200, 70400, 70450, 70500,
    70600,
  ]

  // job : Approved, Invoiced, Paid, Without invoice
  // assign : Declined, Canceled, Unassigned accepted, Unassigned, Unassigned
  const jobTerminatedProStatuses = [
    60600, 60700, 60800, 60900, 70200, 70400, 70450, 70500, 70600,
  ]
    ...
    if (messageType === 'request') {
      if (sendFrom === 'LPM') {
        const selectedJob = jobDetail?.find(
          value => value.jobId === selectedJobId,
        )
        const selectedJobStatus = selectedJob?.jobInfo?.status
        return (
          (jobDetail &&
            !jobDetail?.some(
              value => value.jobId === selectedJobId && value.jobInfo?.pro,
            )) ||
          (selectedJobStatus &&
            !jobTerminatedLPMStatuses.includes(selectedJobStatus))
        )
      } else {
  ...
  const handleReadMessage = () => {
    ...
    if (sendFrom === 'LPM' && messageType === 'job' && jobDetail) {
      ...
      if (
        selectedJob?.jobInfo?.contactPerson?.userId ===
  ...
  return (
    <Box
      sx={{
        maxWidth:
          sendFrom === 'LPM' && messageType === 'job' ? '648px' : '483px',
                  ...
                  <Box
                    ...
                    sx={{
                      ...
                      borderBottom:
                        ...
                        messageList.contents?.[index + 1].messageType ===
                          'system'
                          ? 'none'
                          : '1px solid #E9EAEC',
                    }}
          ...
          >
            ...
            >
              <TextField
                ...
                placeholder={
                  sendFrom === 'LPM'
                    ? 'Write down a message to Pro'
                    : 'Write down a message to LPM'
                }
                value={message ?? ''}
