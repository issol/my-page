...
export const handlers = [
  ...
  rest.get(BASEURL + '/api/enough/onboard/guideline', (req, res, ctx) => {
    ...
    function generateRandomData() {
      const data = [];
      for (let i = 0; i < 20; i++) { // 20개의 랜덤 데이터 생성
    ...
    function filterData(titles?: string, clients?: string[], categories?: string[], serviceTypes?: string[]): Data[] {
      return sampleList.filter((item) =>
        (!titles || titles === item.title) &&
        (clients?.length === 0 || clients?.includes(item.client)) &&
        (categories?.length === 0 || categories?.includes(item.category)) &&
        (serviceTypes?.length === 0 || serviceTypes?.includes(item.serviceType))
      );
    }
