...
export const Default = (args: Props) => {
  ...
  return (
    <div>
      <Alert severity='info'>
        Modal은 <code>ModalContext</code>의 <code>setModal</code> 메소드를 통해
        간단하게 모달을 열 수 있습니다.
        <br />
        모달을 미리 페이지에 심어두고 open값을 조작할 필요 없이, <br />
        모달이 필요한 시점에 <code>setModal</code>함수에 <code>ReactNode</code>
        타입의 엘리먼트를 전달해주면 됩니다.
        <br />
        <strong>
          <code>setModal</code>의 단점은, 부모 컴포넌트로부터 물려받은 state의
          값이 변할 때마다 동기적으로 그 값을 보여줄 수 없다는 것입니다.
          <br />
